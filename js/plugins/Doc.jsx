...
class Doc extends React.Component {
    ...
    getHiglightedValue = name => {
        if (!this.state.filter) {
            return name;
        }
        if (!name.toLowerCase().match(this.state.filter)) {
            return name;
        }
        const splitName = name.toLowerCase().split(this.state.filter);
        return splitName.reduce((a, b, i) => i < splitName.length - 1 ? [...a, <span key={i}>{b}</span>, <strong key={i + '_s'} >{this.state.filter}</strong>] : [...a, <span key={i}>{b}</span>], []);
    }

    render() {

        return (<ReactCSSTransitionGroup
            transitionName="a-doc-transition"
            transitionEnterTimeout={300}
            transitionLeaveTimeout={300}>

            {this.props.enabled && (
            <div key="doc" className="a-doc">
                <div className="a-conatiner">
                    <div className="a-head">
                        <input value={this.state.filter || ''} placeholder="filter docs..." onChange={(e) => {
                            this.setState({
                    ...
                    <div className="a-body-scroll">
                        {Object.keys(references).map((r, i) => {
                            return (
                                <div key={i} className="a-component">
                                    <div className="a-component">
                                        <div className="a-title">{r}</div>
                                        {Object.keys(references[r]).sort().filter(v => v.match(this.state.filter) || head(Object.keys(references[r][v]).filter(a => references[r][v][a].match(this.state.filter)))).map((v, j) => {
                                            return (
                                                <div key={j} className="a-ref">
                                                    <div className="a-title">{v}</div>
                                                    {references[r][v].desc && <div className="a-desc">{this.getHiglightedValue(references[r][v].desc)}</div>}
                                                    {references[r][v].example && <pre>{this.getHiglightedValue(references[r][v].example)}</pre>}
                                                    {references[r][v].selectors && <div className="a-desc">selectors: {this.getHiglightedValue(references[r][v].selectors)}</div>}
                                                    {references[r][v].sld && <div className="a-sld">{references[r][v].sld}</div>}
                                                </div>
                                            );
                                        })}
                                    </div>
...
const docSelector = createSelector([
        state => state.editor && state.editor.docs
    ], (enabled) => ({
        enabled
    })
);

const DocPlugin = connect(docSelector)(Doc);
